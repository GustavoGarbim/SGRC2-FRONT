...
export const cadastrarNovoContrato = async (dados) => {
  const response = await fetch(`${API_BASE_URL}/Contrato`, {
    method: "POST",
...
export const alterarStatusContrato = async (id, ativar) => {
  const url = ativar
    ? `${API_BASE_URL}/Contrato/AtivarContrato/${id}`
    : `${API_BASE_URL}/Contrato/${id}`;
  const method = ativar ? "POST" : "DELETE";

  const response = await fetch(url, { method });
  if (!response.ok)
    throw new Error(`Falha ao ${ativar ? "ativar" : "desativar"} contrato.`);
  return response;
};

export const gerarPlanilhaExcel = async () => {
  const response = await fetch(`${API_BASE_URL}/Contrato/exportar`, {
    method: "GET",
  });
